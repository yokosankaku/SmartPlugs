...
void task_measure_and_current_function( void * pvParameters )
{
  ...
  i2cdev_init();
  ads111x_init_desc(&devices_ads1115, ADS111X_ADDR_GND, I2C_NUM_0, I2C_SDA_PIN, I2C_SCL_PIN);
  ads111x_set_mode(&devices_ads1115, ADS111X_MODE_CONTUNOUS);
  ads111x_set_data_rate(&devices_ads1115, ADS111X_DATA_RATE_860);
  ads111x_set_gain(&devices_ads1115, ADS111X_GAIN_6V144);
  ...
  while(true)
  {
    ...
    numer_measure = 0;
    sum_current = 0;
    ads111x_set_input_mux(&devices_ads1115, ADS111X_MUX_3_GND);
    delay_us(3000);
    ads111x_get_value(&devices_ads1115, &max_adc_value);
    while((esp_timer_get_time() - time_measure_cur) <= 100000)
    {
      ads111x_set_input_mux(&devices_ads1115, ADS111X_MUX_0_GND);
      delay_us(3000);
      ads111x_get_value(&devices_ads1115, &cur_analog_value);
      cur_calibr_sq_value = (max_adc_value / 2) - cur_analog_value;
      sum_current += cur_calibr_sq_value * cur_calibr_sq_value;
      numer_measure += 1;
    }
    primary_settlement_cur = sqrt(sum_current / numer_measure);
    current_value = primary_settlement_cur * 0.00158891145951338857247876994526;
...
static void my_init( void )
{
  ...
  gpio_pad_select_gpio(ADC_SENSOR_RD_PIN);
  gpio_set_direction(ADC_SENSOR_RD_PIN, GPIO_MODE_INPUT);
  gpio_pad_pullup(ADC_SENSOR_RD_PIN);  
